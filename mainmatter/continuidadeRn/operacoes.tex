...
\begin{proof}
    ...
    (b) Se $\lambda=0$, então $\lambda f$ é a função constante igual a $0$, e o limite é $0=\lambda L$.
    ...
    (c) Este item é consequência dos dois itens já provados, pois
    \[
        f-g=f+(-1)g.
    \]
    Assim,
...
\begin{corollary}\index{continuidade!soma}\index{continuidade!diferença}\index{continuidade!multiplicação por escalar}
    ...
    Se $f$ e $g$ são contínuas em $p$, então $f+g$ e $f-g$ são contínuas em $p$.
...
\begin{example}
    ...
    \[
        F(x,y)=x+2y
    \]
    $F$ é contínua, pois é a soma da projeção $(x,y)\mapsto x$ com a multiplicação por escalar $2$ da projeção $(x,y)\mapsto y$.
\end{example}
