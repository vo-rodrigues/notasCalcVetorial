...
\begin{proposition}
    Sejam:
    \begin{itemize}
        \item $M, N, P$ espaços métricos (como $\mathbb R^n$ e $\mathbb R^m$ e $\mathbb R^k$),
        \item $s\in A\subseteq M$ um conjunto,
        \item $p \in B\subseteq N$ um conjunto,
        \item $\gamma: A \to B$ uma função,
        \item $f: B \to \mathbb R^k$ uma função,
        \item $p=\lim_{t\to s} \gamma(t)$, ponto de acumulação de $B$,
        \item $L=\lim_{x\to p} f(x)$.
    \end{itemize}
    Suponha que $\gamma(t)\neq p$ para todo $t \in A\setminus\{s\}$.
    Então
    \[
        \lim_{t\to s} (f\circ \gamma)(t) = L.
    \]
\end{proposition}