...
\begin{proposition}
    ...
    Em outras palavras, se $\lim_{x\to a} f(x)$ existe e pertence a $B$, e $g$ é uma função contínua em $\lim_{x\to a} f(x)$, então
    \[
...
\begin{proof}
    ...
    Fixe $\epsilon>0$.
    Como $L$ é limite de $g$ em $b$, existe $\nu>0$ tal que, para todo $y \in B$, se $d_N(y, b) < \nu$, então $d_P(g(y), L) < \epsilon$.
