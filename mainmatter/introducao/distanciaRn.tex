...
Para as dimensões $n=1$, $n=2$ e $n=3$, essa é a conhecida distância usual entre dois pontos estudada na Geometria Analítica.
Para dimensões maiores, tal distância pode ser encarada como uma generalização, por enquanto.
Um teorema profundo da Análise garante que as propriedades topológicas advindas de distâncias em $\mathbb R^n$ com certas propriedades naturais são equivalentes, de modo que tal generalização fica teoricamente justificada.
...
\begin{knowMore}
    Um espaço métrico é um par $(X, d)$ onde $X$ é um conjunto e $d: X\times X \to \mathbb R$ é uma função que satisfaz as propriedades acima.
\end{knowMore}
