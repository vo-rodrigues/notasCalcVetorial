...
Não é nosso objetivo, nesta seção, apresentar uma introdução completa à Topologia, mas apenas apresentar noções topológicas básicas que serão úteis para o estudo de limites, continuidade e diferenciabilidade em $\mathbb R^n$.
...
\subsection{Pontos isolados e de acumulação de subconjuntos de \texorpdfstring{$\mathbb R^n$}{Rn}}
...
\begin{definition}\index{ponto isolado}
    ...
    \begin{itemize}
        \item $n\geq 1$,
        \item $A\subseteq \mathbb R^n$, e\
        \item $p \in A$.
    \end{itemize}
    Dizemos que um ponto $p \in A$ é um \emph{ponto isolado} de $A$ se existe $r>0$ tal que
    \[
        B(p, r)\cap A = \{p\}.
    \]
\end{definition}
...
\begin{definition}\index{ponto de acumulação}
    ...
    \end{itemize}
    Dizemos que um ponto $p \in \mathbb R^n$ é um \emph{ponto de acumulação} de $A$ se, para todo $r>0$,
...
\subsection{Subconjuntos fechados de \texorpdfstring{$\mathbb R^n$}{Rn}}
