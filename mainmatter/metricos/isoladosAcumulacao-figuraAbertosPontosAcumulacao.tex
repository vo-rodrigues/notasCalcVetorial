\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        % Um ponto q diferente de p dentro da bola menor.
        \draw[Red!85!black, thick, ->] (p) -- (q)
            node[midway, above left] {$q\neq p$};
        \filldraw[black] (p) circle (1.9pt) node[below left] {$p$};
        \filldraw[Red!85!black] (q) circle (2pt) node[above right] {$q$};

        \node at (2.55,-2.05) {$s'=\min\{s,r\}$};
    \end{tikzpicture}
    \caption{A bola $B(p,s')$ fica dentro de $A$ e, quando o espaço não possui pontos isolados, contém um ponto $q\neq p$.}
\end{figure}
